...
<table class="mainTable" cellpadding="0" cellspacing="0" align="center" height="95%">
	...
	<tr height="*">
			<td><c:import url="./navigation.jsp" /></td>
			<td>
	...
	<c:if test="${!empty searchPhotos && !empty photoPages}">
		 ...
		 <table class="tabsMiddle top10" cellpadding="0" cellspacing="0" align="center">
				...
				<tr>
					...
					<div style="width: 80%;" align="left"><c:forEach var="photo"
						items="${photoPages[pageIndex].photos}">
						<div class="smallestProfile">
						<div class="smallProfilePicOffline"><a
							href="./photo?action=LOAD&photoId=${photo.photoId}"> <img
							class="pic" src="./image?photoId=${photo.photoId}"
							alt=${photo.phName } title=${photo.phName } /> <!-- <img class="pic" src="${photoPages[pageIndex].absolutePaths[photo.path]}" alt=${photo.phName} title=${photo.phName} /> -->
						</a></div>
						<div class="vipPic">&nbsp;</div>
						<div class="lh17"><a
							href="./photo?action=LOAD&photoId=${photo.photoId}"
							class="link bold">Номер: ${photo.photoId}</a></div>
				...
				<tr>
					...
					<div class="fLeft left10"><c:if test="${pageIndex > 0}">
						<a href="./search?action=PREV_PAGE"><img src="./images/btnLeft.gif" /></a>
					</c:if> <c:if test="${pageIndex + 1 < totalPages}">
						<a href="./search?action=NEXT_PAGE"><img
							src="./images/btnRight.gif" /></a>
					</c:if> <span style="vertical-align: 50%">${pageIndex + 1} от ${totalPages}</span></div>
					</td>
				</tr>
			</table>
		</td>
			...
			<c:if test="${empty searchPhotos && empty photoPages}">
			<div style="font-size: 30px;">Нама намерени снимки с това име</div>
			</c:if>
